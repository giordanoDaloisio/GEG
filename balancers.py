...
class MulticlassBalancer:
    def __init__(self,
        ...
        self.p_y_a = np.array([tools.p_vec(y[ids], all_classes=self.outcomes)
                               for ids in group_ids])
        
        # Getting the group-specific P(Y), P(Y- | Y), and constraint matrices
        p_vecs = np.array([tools.p_vec(y[ids], all_classes=self.outcomes) for ids in group_ids])
        p_pred_vecs = np.array([tools.p_vec(y_[ids], all_classes=self.outcomes) for ids in group_ids])
        ...
        self.cp_mats = np.array([tools.cp_mat(y[ids], y_[ids], all_classes=self.outcomes) 
                                 for ids in group_ids])
        ...
        self.cp_mat = tools.cp_mat(y, y_, all_classes=self.outcomes)
