...
def cp_mat(y, y_, all_classes=None):
    '''Returns the matrix of conditional probabilities y_ | y
    
    Parameters:
        y: array-like, true labels
        y_: array-like, predicted labels  
        all_classes: array-like, optional. If provided, ensures the output
                     matrix has rows/columns for all classes
    '''
    if all_classes is not None:
        # Create crosstab with all classes as index and columns
        tab = pd.crosstab(pd.Categorical(y, categories=all_classes), 
                          pd.Categorical(y_, categories=all_classes),
                          dropna=False).values
    else:
        tab = pd.crosstab(y, y_, dropna=False).values
...
def p_vec(y, flatten=True, all_classes=None):
    '''Returns the matrix of probabilities for the levels y
    
    Parameters:
        y: array-like, the data to compute probabilities for
        flatten: bool, whether to flatten the output
        all_classes: array-like, optional. If provided, ensures the output
                     has an entry for each class, even if not present in y
    '''
    if all_classes is not None:
        # Count occurrences for all classes, including those not in y
        counts = pd.Series(y).value_counts()
        tab = np.array([counts.get(c, 0) for c in all_classes], dtype=float)
    else:
        tab = pd.Series(y).value_counts().sort_index().values.astype(float)
